...
name: map-module
description: >
  Create, fully refresh, or incrementally maintain a source-verified subsystem architecture map
  under `.claude/maps/`. Use when the user says "/map-module", "map this module", "map the
  <subsystem> architecture", asks to document how a subsystem works today, or when project
  instructions (the Knowledge Loop Conventions in CLAUDE.md) require a targeted refresh after
  implementation changes invalidate mapped facts. Do not use for ad hoc code explanation, a
  single-file question, or changes that leave the existing map accurate. Authorizes bounded,
  read-only Explore subagents for research and verification.
---

# Map Module

Produce a "how it works today" snapshot. Keep decision history in the paired
`.claude/decisions/<module-id>.md`; do not turn the map into a design proposal or changelog.
...
- Keep synthesis, verdict judgment, file edits, module registration, and the final response in the
  root agent.
- Use read-only `Explore` subagents (the Agent tool with `subagent_type: Explore`) only for code
  research and verification. Full mapping uses the research and verification waves below. A
  bounded targeted refresh may be verified locally; use explorers when the affected flow crosses
  concerns or repositories. Tell every explorer not to edit files.
- Run explorers in bounded waves: launch one wave's agents in a single message so they run
  concurrently, wait for the wave to report, then dispatch the next. Reuse an explorer through
  `SendMessage` only for a related concern.
...
2. Read `.claude/MODULES.md`, the paired decision file if present, and the current map if refreshing.
...
4. Inventory the root with the Glob tool (or `rg --files`), then inspect class declarations,
   constructors or registration points, public entry points, core state fields, and file sizes.
   Skim key files to confirm boundaries.
...
4. Synthesize a baseline map using the final map template. Merge overlaps, distinguish public
   ...
   `To verify`.
5. Write or update `.claude/maps/<module-id>.md` with the `Write` tool (new file) or the `Edit`
   tool (existing file).
...
Use the `Edit` tool for patches. Match the existing text exactly, punctuation included, when
replacing it. For large section rewrites, anchor on unique headings; if a match is ambiguous, widen
the anchor rather than guessing.

## Phase 4: Register, connect, and validate

1. If the module is new, add it to the matching section of `.claude/MODULES.md` only after the map
   is verified. Keep the registry near its documented size and describe the verified boundary.
2. Check whether the root `CLAUDE.md` tells Claude Code to resolve affected module IDs, read the
   matching map and decision files before editing, and conditionally run a targeted map refresh
   when implementation changes invalidate mapped facts. If those rules are missing or only mention
   decisions, do not edit `CLAUDE.md`. Tell the user that `/commit-context` will propose the
   canonical Knowledge Loop Conventions upgrade when the map is staged for commit. If
...
3. Confirm that the final map:
   ...
   - pairs to the same module id under `.claude/decisions/`.
