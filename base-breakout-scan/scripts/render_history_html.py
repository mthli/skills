...
const I18N = {
  en: {
    ...
    seNote: minN => `Average result per completed trade, grouped by the name's sector: the ledger's read on whether it matters what kind of stock the base sits in.\nThe capped line above each bar is its 95% interval. A sector whose interval reaches the dashed all-trades average reads no different from the board. Sectors under ${minN} completed trades fold away.\nA gap here is as likely to be the last two months of sector beta as a durable property of these bases: an observation to re-check each quarter, not a filter.`,
    ...
    seTipWorse: "Trails the all-trades average",
