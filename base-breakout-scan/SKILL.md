...
1. **Base length > 20 weeks is the one big validated edge.** +4.9% avg at +20 sessions, 75% win rate, 14% stop-hit, 76% still above pivot at +10 (n=28 across 24 distinct tickers). Adding "don't chase fills >3% above pivot" lifts it to **+5.5%/trade vs −0.9% baseline**. Sub-10-week bases: 37% win, −4.6%. The O'Neil "bigger base → bigger move" claim survives contact with data; fresh bases are where the failures live.
2. **Volume dry-up is INVERTED vs doctrine in this sample.** Vol↓ ≥ 0.90 (no dry-up): +0.4% avg, 52% win, 37% stop-hit. Deep dry-up < 0.70: **−8.8%, 33% win, 61% stop-hit**. The toxic combo — base < 10wk AND Vol↓ < 0.9 — ran −8.6% with 82% stop-hit (n=38). Mirrors the momentum-scan entry-volume finding: quiet volume in 2026's tape means dead, not coiled. Doctrine-contradicting, so re-validate as history grows before hard-coding.
...
4. **Baseline is no-edge: the watchlist as a whole did not pay.** 49% of episodes touch their pivot (median 1 session); post-trigger avg −2.6%, 45% win, **66% fall back below the pivot within 5 sessions**; the stop-based trade averaged −0.9% with the stop hit 51% of the time. The literature 40-50% win rate reproduced — but without the asymmetric payoff, so selection (which base) is everything.
...
6. **Volume confirmation did not rescue entries.** Close-above-pivot on ≥1.5× volume, entering next open: 35% win, −8.1% avg (n=26, small). Confirmation costs next-open slippage and selected no better in this tape. Bare-touch vs close-based entries were near-identical.
...
8. **First appearance far from pivot is a fade signal.** Names entering the list ≥−3% from pivot: 70% trigger rate, breakeven trades. Entering at −10..−3%: 28% trigger, −3.8%/trade when they finally grind up. Also: triggers taking 6-10 sessions to fire went 22% win / 89% stop-hit (n=9) — a base that takes that long to reach a pivot it started near arrives exhausted.
