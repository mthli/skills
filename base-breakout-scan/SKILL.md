...
`scripts/backtest_outcomes.py` replays `state/history.csv` as the canonical trade: while a name stays on the watchlist, a buy-stop sits at the most recent pivot (fill = max(pivot, open) so gaps pay real slippage); dropout cancels the order. Measured on 426 testable episodes (142 triggered with a full +20-session window) over a flat tape (SPY forward-20d ≈ 0%, regime mostly RISK-ON/CAUTION). Findings, strongest first:

1. **Base length ≥ 20 weeks is the one big validated edge.** +4.9% avg at +20 sessions, 75% win rate, 14% stop-hit, 76% still above pivot at +10 (n=28 across 26 distinct tickers); as the stop-based trade that's **+4.9%/trade vs −0.8% baseline**. Notably, no ≥20-week trigger gapped more than 3% above its pivot in-sample — the gap-chase failure mode (#5) lives entirely in the short-base names. Sub-10-week bases: 37% win, −4.6%. The O'Neil "bigger base → bigger move" claim survives contact with data; fresh bases are where the failures live.
2. **Volume dry-up is INVERTED vs doctrine in this sample.** Vol↓ ≥ 0.90 (no dry-up): +0.4% avg, 52% win, 37% stop-hit. Deep dry-up < 0.70: **−8.8%, 33% win, 61% stop-hit**. The toxic combo — base < 10wk AND Vol↓ < 0.9 — ran −8.6% with 79% stop-hit (n=38). Mirrors the momentum-scan entry-volume finding: quiet volume in 2026's tape means dead, not coiled. Doctrine-contradicting, so re-validate as history grows before hard-coding.
3. **The composite Base Score does not discriminate outcomes.** <50 / 50–59 / 60–69 bands are statistically identical; the four ≥70 episodes never even triggered. Its two biggest weights (tightness 25pts, dry-up 15pts) pointed nowhere or backwards in this sample. Treat Score as a display floor, rank by BaseWks instead.
...
6. **Volume confirmation did not rescue entries.** Close-above-pivot on ≥1.5× volume, entering next open: 33% win, −6.3% avg (n=21, small). Confirmation costs next-open slippage (median 2.2% over pivot) and selected no better in this tape. Close-based entries without the volume gate ran marginally ahead of bare-touch (−0.3% vs −0.8%/trade) but pay a 1.5% median entry premium — neither changes any conclusion.
7. **RS slope has a sweet spot, not a gradient.** 0–1%/wk: +0.3%, 57% win. **>1%/wk: −6.5%, 67% stop-hit** — a stock outrunning SPY that hard while "ranging" is usually extended high-beta, not quiet accumulation. ≤0: 36% win. Prefer the calm-positive middle.
...
2. **🔥 IMMINENT names beat 🚀 BREAKOUTs in expected value for most users.** Counterintuitively: 🚀 means the breakout already happened, so you're chasing 1-2% above the pivot with all the risk and less of the move. 🔥 means the trigger hasn't fired yet — you can set a buy-stop *at* the pivot, eliminating the "chase" cost. The entry-mechanics logic holds, but don't oversell the signal itself: in the 2026 backtest 🔥-at-start episodes performed no better than 📊 ones (the 🔥 state requires deep dry-up, which was inverted — see Backtested outcomes #2), and episodes that *reached* 🔥 fell back below the pivot within 5 sessions 85% of the time. Also, the backtest found RSslope > +1%/wk during a base was a *negative*, not a positive — prefer the calm 0-1%/wk band.
...
5. **Maturing bases (streak ≥ 4) are the real signals — with a nuance the backtest added.** Long persistence means the base has held through actual market noise. But note what the outcome data actually validated: *base length in weeks* (BaseWks ≥ 20) was the strong predictor; *time sitting on the watchlist before triggering* was not protective (triggers that took 6-10 sessions to fire went 22% win). So prefer a name that's been consolidating for months over one that's merely been on our list for days — streak is a durability check, not an edge by itself.
