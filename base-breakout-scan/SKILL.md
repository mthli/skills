...
_Diagnostic columns (RS, Smooth%, BB%ile, Vol↓, RSslope%/wk, VCP, RankΔ, FirstSeen): --verbose_
...
- **VCP** (*verbose*): the base's Volatility Contraction Pattern geometry as a contraction-depth chain plus verdict. `18.2>9.1>4.0 ✓` = three successive pullbacks of 18.2%, 9.1%, 4.0% inside the base, and the sequence passes the geometric VCP test (2-6 contractions, non-increasing within a 15% wobble, at least halving overall, final leg ≤ 10%); `✗` = a sequence exists but fails the test; `—` = no completed contractions (flat or single-leg range). **Hypothesis column, not a signal**: nothing ranks or scores on it and no outcome data validates it yet — recorded since 2026-08-03 precisely so the quarterly backtest can judge it (see **VCP annotation** below). JSON carries the components: `vcp_contractions`, `vcp_depths`, `vcp_ratio`, `vcp_last_depth_pct`, `vcp_handle_pivot`, `vcp_vol_contraction`, `vcp_is_vcp`.
...
## VCP annotation (hypothesis, collecting since 2026-08-03)

Every detected base gets a Volatility Contraction Pattern annotation: the base window is zigzagged into swing highs/lows, each completed high→low leg counts as one "contraction", and `vcp_is_vcp` asks whether the depths shrink left to right the way Minervini draws it (2-6 contractions, non-increasing within a 15% wobble, at least halving overall, final leg in single digits). Persisted per pick to `history.csv`; shown in the `--verbose` table (VCP column) and the `--ticker` Stage 2 report.

Deliberate design choices:

- **It's a recording, not a signal.** The Score doesn't read it, ranking doesn't read it, no ⭐ treatment. VCP doctrine is tightness + dry-up, and this scan's own outcome backtest measured the tightness composite as non-discriminating (#3) and deep dry-up as inverted (#2) — so the doctrine enters as a hypothesis, the way any new attribute should.
- **The boolean is geometry-only.** The doctrine's volume half lands in its own column (`vcp_vol_contraction`, last-leg vs first-leg mean volume) so the backtest can judge the two halves independently instead of baking finding #2's indicted signal into the flag.
...
- **Base-detection is a screener heuristic, not a chart annotator.** The trailing-window approach (find the longest window where price range ≤ max_base_width) catches the same population as Minervini's VCP / O'Neil's cup-and-handle without trying to identify those exact patterns. (The 2026-08-03 **VCP annotation** measures the contraction geometry *inside* an already-detected window — it refines the description of what was caught; it doesn't change which windows get caught.) Real chartists can find textbook bases this misses (e.g., a stair-stepping handle inside a larger cup), and vice versa: a window that meets the numerical criteria may be a *failing* base by eye (descending highs, no proper handle). Use the surfaced names as candidates to chart, not as finished analysis.
