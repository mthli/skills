...
`scripts/backtest_outcomes.py` replays `state/history.csv` as the canonical trade: while a name stays on the watchlist, a buy-stop sits at the most recent pivot (gaps pay real slippage); dropout cancels the order. 426 testable episodes over a flat tape; re-run quarterly. Findings, strongest first — full evidence, magnitudes, and caveats in `references/backtest-findings.md` (the numbering below is referenced throughout this file):

1. **Base length ≥ 20 weeks is the one big validated edge** — +4.9%/trade, 75% win vs −0.8% baseline; sub-10-week bases are where the failures live. The ★ validated pocket.
...
3. **The composite Base Score does not discriminate outcomes** — bands statistically identical; treat Score as a display floor, rank by BaseWks.
...
8. **First appearance far from pivot is a fade signal** — entering at −10..−3% from pivot: 28% trigger rate, −3.4%/trade; slow triggers (6-10 sessions) arrive exhausted.
