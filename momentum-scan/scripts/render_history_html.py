...
/* A line series keys with a stroke, not a box: the bump chart's mark IS a
   stroke, and a filled swatch would key it with a shape the chart never
   draws. The two filled panels (sector stack, heatmap) keep the box. */
...
/* The date rides at the far right of the header row, muted: it identifies
   the point but is not what the reader hovered for. */
...
/* The point's numbers, aligned. Value wears the ink and the label the muted
   step — in a tooltip the reader already knows what they hovered and came
   for the number. Deltas ride in the value, so Score / Return / Drawdown
   stop sharing a line and the parentheses line up under each other. */
...
const I18N = {
  en: {
    ...
    gapNote: g => `Deltas vs ${g} days earlier`,
...
const gapNote = g => g > 1 ? T.gapNote(g) : null;
// A count change reads as a direction, so an unchanged sector says so with
...
const BUMP_MIN_APPS = 5;
{
  ...
  svg.addEventListener("pointermove", ev => {
    ...
    if (best) {
      ...
      const p = best.p;
      showTip(ev.clientX, ev.clientY, t => tipCard(t, {
        title: best.s.t,
        aux: DATA.days[d],
        color: slotColorOf(best.s.t) || "var(--ctx-line)",
        line: true,
        // The rank in words, because the line's height cannot always say
        // it: everything below the board's cutoff is clamped onto the
        // dashed floor, so #34 and #58 sit at the same y.
        sub: rankTxt(p.r),
        // One row each. Sharing two lines put Score next to Return and
        // stranded Drawdown, when the reading that matters is vertical:
        // whether the Score move came from return added or drawdown taken.
        kv: [
          [T.score, p.s.toFixed(2) + (pp ? dlt(p.s - pp.s, 2) : "")],
  ...
  const unpin = () => { pinned = null; restyle(null); renderDetail(det, null); };
  svg.addEventListener("click", () => { pinned = (pinned === lit ? null : lit); restyle(pinned); renderDetail(det, pinned, unpin); });
}
...
// ---- sector stacked columns ----
{
  ...
  svg.addEventListener("pointermove", ev => {
    ...
    if (t.tagName === "rect" && t.dataset.i !== undefined) {
      const i = +t.dataset.i, d = +t.dataset.d, c = +t.dataset.c;
      const counts = DATA.sectors.perDay[d];
      const prev = d > 0 ? DATA.sectors.perDay[d - 1][i] : null;
      const fillOf = j => j < TOPSEC ? `var(${SLOTS[j]})` : "var(--other)";
      showTip(ev.clientX, ev.clientY, tt => tipCard(tt, {
        title: `${secName(names[i])} ${T.names(c)}`,
        aux: DATA.days[d],
        color: fillOf(i),
        // The whole column, not just the segment under the cursor. The
        // panel asks which sector is taking over the board, and that is a
        // comparison between segments — each of which currently costs its
        // own hover to give up a number.
        kv: counts.map((n, j) => n ? [secName(names[j]), `${n}`] : null),
        // The move, which no single column can show: eight seats is a
        // sector arriving or leaving depending on where it was yesterday.
        notes: [prev === null ? null : T.secDelta(sgnTxt(c - prev))],
      }));
    } else hideTip();
  });
...
let heatPinned = null;
const heatUnpin = () => { heatPinned = null; renderDetail(heatDet, null); highlightHeatRow(); };
...
function renderHeat(minApps) {
  ...
  rows.forEach((s, ri) => {
    ...
    for (const [pi, p] of s.pts.entries()) {
      ...
      const r = el("rect", {x: GL + p.d*CW, y: y+1, width: CW-2, height: CH-2, rx: 2,
        fill}, svg);
      // Carried on the cell so its tip can key with the exact shade the
      // cursor is on, rather than re-deriving the bucket from the rank.
      r.dataset.f = fill;
  ...
  svg.addEventListener("pointermove", ev => {
    ...
    if (t.tagName === "rect" && t.dataset.t) {
      ...
      showTip(ev.clientX, ev.clientY, tt => tipCard(tt, {
        ...
        aux: DATA.days[+c.d],
        color: c.f,
        // Same card as the trajectories, so the two panels read the same
        // way: the shade said the rank band, this names the rank — and
        // says when it is below the cutoff, which the palest cell cannot.
        sub: rankTxt(+c.r),
        kv: [
          [T.score, (+c.s).toFixed(2) + (c.ds ? dlt(+c.ds, 2) : "")],
  ...
  svg.addEventListener("click", ev => {
    ...
    renderDetail(heatDet, heatPinned, heatUnpin);
    highlightHeatRow();
  });
  highlightHeatRow();
}
