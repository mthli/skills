...
| `calendars` | ~0.7–2 s per `--type` | **Per-call, not per-ticker** — calendars is market-wide, no ticker iteration. One HTTP per `--type` (the underlying yfinance `Calendars.get_*_calendar` is a single POST to `/v1/finance/visualization`), **except `--type earnings` with the default most-active filter, which is 2 HTTP** (the prescreen calls Yahoo's `MOST_ACTIVES` predefined screener with `count=200` first, then the calendar query). Pass `--no-most-active` to skip the prescreen. Multi-type (`--type earnings,ipo,splits` or `--type all`) sequences the per-type HTTP calls — total ≈ N × per-type cost (so `--type all` with default earnings filter is 5 HTTP). `--summary` is post-fetch (no latency change). `--full` is also post-fetch. `--past-days N` doesn't change cost; just shifts the window. |
