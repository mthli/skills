...
description: "Scan US large-cap equities for short-term oversold reversals — Connors-style RSI(2) setups inside confirmed long-term uptrends. Use when the user wants to find oversold bounces, mean-reversion entries, RSI(2) triggers, short-term pullbacks in strong stocks, names trading below their 5-day average ready to snap back, or a 'buy the dip' watchlist. Triggers on 'find oversold bounces', 'RSI(2) setup', 'mean reversion candidates', 'short-term pullback', 'buy the dip', 'bounce candidates', 'panic sellers', 'overdone sell-off'. The complement to momentum-scan and base-breakout-scan: those find what's running and what's about to run; this finds what just got punched in the face but is structurally fine. Re-runs and parameter tweaks count too. Do NOT use for single-ticker chart analysis (use yfinance), value/contrarian long-term picks, ETF screening, or generic explanations of mean-reversion theory."
---
