...
const I18N = {
  en: {
    ...
    seNote: minN => `Average result per resolved signal, grouped by the name's sector: the ledger's read on whether it matters what kind of stock this is.\nThe capped line above each bar is its 95% interval. A sector whose interval reaches the dashed all-signals average reads no different from the board, and most still reach it. Sectors under ${minN} resolved signals fold away.\nA gap here is as likely to be the last two months of sector beta as a durable property of these names: an observation to re-check each quarter, not a filter.`,
    ...
    seTipWorse: "Trails the all-signals average",
