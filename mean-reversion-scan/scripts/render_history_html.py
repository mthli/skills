...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div class="detail" id="grid-detail"></div>
  </div>
...
const I18N = {
  en: {
    ...
    gridNote: () => `One row per name, one cell per listed day, colored by that day's eventual outcome. A center dot marks ⭐ pocket days (Score ≥ ${DATA.pocket.minScore} on day ≤ ${DATA.pocket.maxStreak} of the spell). Rows unbroken for ≥ __STUCK_MIN_STREAK__ days are stuck oversold — the bounce never came; a warning, not a bargain.\nClick a row to open its RSI(2) / Score / Result strip.`,
    gridFilterLabel: "Filter by days listed",
    geDays: n => `≥${n} days`, all: "All",
    pkTitle: "⭐ Pocket vs the rest",
    pkNote: "Running expectancy — average %/signal over all resolved signals to date — of ⭐ pocket signals vs everything else. Dashes mark the 2026-05→07 backtest's in-sample values: the pocket line holding above its dash means the validated edge keeps paying out-of-sample.",
  ...
  zh: {
    ...
    gridTitle: "结局网格",
    gridNote: () => `每个标的一行，每个上榜日一格，颜色 = 该日信号的最终结局。带中心点的格子是 ⭐ 口袋日（Score ≥ ${DATA.pocket.minScore} 且上榜 ≤ ${DATA.pocket.maxStreak} 天）。连续 ≥ __STUCK_MIN_STREAK__ 天不断的长行就是卡死超卖——反弹一直没来，是警告不是便宜货。\n点击任意行展开其 RSI(2) / 评分 / 盈亏走势。`,
    gridFilterLabel: "按上榜天数筛选",
    geDays: n => `≥${n} 天`, all: "全部",
    pkTitle: "⭐ 口袋 vs 其余",
    pkNote: "滚动期望——截至当日全部已结算信号的平均盈亏 %/信号——⭐ 口袋信号与其余信号各一条线。虚线为 2026-05→07 回测的样本内数值：口袋线稳在自己的虚线上方，说明验证过的边际在样本外仍在兑现。",
  ...
  zht: {
    ...
    gridTitle: "結局網格",
    gridNote: () => `每個標的一行，每個上榜日一格，顏色 = 該日訊號的最終結局。帶中心點的格子是 ⭐ 口袋日（Score ≥ ${DATA.pocket.minScore} 且上榜 ≤ ${DATA.pocket.maxStreak} 天）。連續 ≥ __STUCK_MIN_STREAK__ 天不斷的長行就是卡死超賣——反彈一直沒來，是警告不是便宜貨。\n點擊任意行展開其 RSI(2) / 評分 / 盈虧走勢。`,
    gridFilterLabel: "按上榜天數篩選",
    geDays: n => `≥${n} 天`, all: "全部",
    pkTitle: "⭐ 口袋 vs 其餘",
    pkNote: "滾動期望——截至當日全部已結算訊號的平均盈虧 %/訊號——⭐ 口袋訊號與其餘訊號各一條線。虛線為 2026-05→07 回測的樣本內數值：口袋線穩在自己的虛線上方，說明驗證過的邊際在樣本外仍在兌現。",
  ...
  ja: {
    ...
    gridTitle: "結果グリッド",
    gridNote: () => `1 銘柄 1 行、リスト入りした日ごとに 1 セル、色 = その日のシグナルの最終結果。中心に点があるセルは ⭐ ポケット日（Score ≥ ${DATA.pocket.minScore} かつ ${DATA.pocket.maxStreak} 日目以内）。__STUCK_MIN_STREAK__ 日以上途切れない長い行は停滞した売られすぎ——反発が来なかった警告であり、掘り出し物ではない。\n行をクリックすると RSI(2) / スコア / 損益の推移が開きます。`,
    gridFilterLabel: "リスト入り日数で絞り込み",
    geDays: n => `${n} 日以上`, all: "すべて",
    pkTitle: "⭐ ポケット vs その他",
    pkNote: "ローリング期待値——その日までに確定した全シグナルの平均損益 %/シグナル——を ⭐ ポケットとその他で 1 本ずつ。破線は 2026-05→07 バックテストのイン・サンプル値：ポケット線が自身の破線の上を維持していれば、検証済みのエッジがアウト・オブ・サンプルでも機能し続けている。",
  ...
  ko: {
    ...
    gridTitle: "결과 그리드",
    gridNote: () => `종목당 1행, 등재일마다 1셀, 색 = 그날 신호의 최종 결과. 중심에 점이 있는 셀은 ⭐ 포켓일 (Score ≥ ${DATA.pocket.minScore}, 등재 ${DATA.pocket.maxStreak}일 이내). __STUCK_MIN_STREAK__일 이상 끊기지 않는 긴 행은 정체된 과매도 — 반등이 오지 않은 경고이지 헐값이 아닙니다.\n행을 클릭하면 RSI(2) / 점수 / 손익 추이가 열립니다.`,
    gridFilterLabel: "등재 일수로 필터",
    geDays: n => `${n}일 이상`, all: "전체",
    pkTitle: "⭐ 포켓 vs 나머지",
    pkNote: "롤링 기대값 — 그날까지 확정된 모든 신호의 평균 손익 %/신호 — ⭐ 포켓과 나머지 각 1줄. 점선은 2026-05→07 백테스트의 인샘플 값: 포켓 선이 자기 점선 위를 유지하면 검증된 엣지가 아웃오브샘플에서도 유효하다는 뜻입니다.",
  ...
  svg.addEventListener("pointermove", ev => {
    ...
    if (t.tagName === "rect" && t.dataset.i !== undefined) {
      ...
      showTip(ev.clientX, ev.clientY, tt => tipRows(tt,
        [T.sigCount(+t.dataset.c), `${T.oc[cat]} · ${DATA.days[d]}`,
         `${DATA.days[d]}: ${T.sigCount(total)}`],
...
// ---- ticker detail strip (shared by the grid) ----
function renderDetail(det, t, onClose) {
  ...
  if (onClose) {
    ...
    x.type = "button"; x.className = "dt-close"; x.textContent = "×";
    x.title = T.detClose; x.setAttribute("aria-label", T.detClose);
  ...
  [["RSI(2)", p => p.rsi, v => v.toFixed(1)],
   [T.score, p => p.sc, v => v.toFixed(0)],
   [T.result, p => p.pct, v => pctTxt(v)]].forEach(([lbl, get, fmt]) => {
    const pts = s.pts.filter(p => get(p) != null);
    const m = div("mini", minis);
    const l = div("mlbl", m, lbl);
    const b = document.createElement("b");
    b.textContent = pts.length ? fmt(get(pts[pts.length - 1])) : T.grNoRes;
    l.appendChild(b);
...
const gridDet = document.getElementById("grid-detail");
let gridPinned = null;
const gridUnpin = () => { gridPinned = null; renderDetail(gridDet, null); highlightGridRow(); };
...
function renderGrid(minDays) {
  ...
  svg.addEventListener("click", ev => {
    const t = ev.target;
    if (t.tagName === "rect" && t.dataset.t) {
      gridPinned = (gridPinned === t.dataset.t ? null : t.dataset.t);
    } else if (t.tagName !== "text") {
      gridPinned = null;
    }
    renderDetail(gridDet, gridPinned, gridUnpin);
    highlightGridRow();
  });
  highlightGridRow();
}
...
// ---- roster table ----
{
  ...
  const COLS = [
    ...
    { h: T.cols[2], v: s => s.days,   dir: -1 },
    { h: T.cols[3], v: s => s.pd || null, dir: -1 },
