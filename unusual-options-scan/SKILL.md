...
3. **Cluster size is inverted vs the old doctrine.** 1 flagged contract: −0.58% xT+5 · 2–4: −0.94% · 5–9: −1.10% · **10+: −2.01% (only 38% beat the universe)**. A whole flagged strike-ladder in EOD data marks a name the whole market is speculating on, not a quiet institutional sweep. Read `+N more` as a *froth multiplier*, not conviction.
...
5. **Post-rip flags are a fade tell.** Flags on names up >+10% in the prior 5 sessions: **−2.36% xT+5, −5.92% xT+10, |T+5| = 8.9%, only 38% beating the universe** — the single worst cell in the sample. Post-dump names (≤−5%) also lag (−0.96% xT+5). Even flat-entry names underperform mildly (−0.45%), so the drag isn't *only* reversal chasing — but chasing a rip that options flow "confirmed" was the most reliable way to lose.
...
**Caveats**: one ~2.3-month window, one regime; ticker-days cluster heavily (one flow episode → many rows, all names share each session's tape — excess-vs-universe removes the day effect, nothing removes the episode effect); the flagged pool structurally skews to high-vol names, so part of the underperformance may be a style effect of this particular tape (the flat-pre-move bucket's −0.45% is the cleanest read of the flow-specific component); chronicity is computed full-sample (approximate in live use with trailing appearance counts); the current-universe baseline adds survivorship optimism and contains the flagged names themselves (~10–20% of the pool daily), so the measured underperformance is mildly conservative; bucket cutoffs chosen in-sample. Re-run quarterly — especially #1's sign (a froth-marker in a RISK-ON tape may behave differently in a selloff) and #4's chronic-name drag.
...
1. **Frame the list as an attention/volatility detector, never as a buy list.** The validated read (**Backtested outcomes** #1, #2): these names will likely move more than the market (+26% more absolute movement over 5 sessions) while *underperforming* it on average (−0.9% xT+5, −2.4% xT+10), and the call/put side of the flow tells you nothing about direction. Two legitimate uses: a research funnel for "why is someone positioning here?", and a **de-risking prompt** — if the user *holds* a name that shows up right after a +10% week, that bucket ran −2.4% xT+5 with only 38% beating the universe in-sample (#5); a big flagged cluster on it is a further, independently-measured negative (#3).
2. **Discount chronic residents; prioritize episodic appearances.** A name in the list most days (TSLA/NVDA-class) is structural options-casino volume — the worst-performing pocket (#4). A name that almost never appears and suddenly does is the only bucket that held up. Check the Streak column; when in doubt, `--show-history` prints a **Chronic residents** list (per-ticker share of all snapshots) — discount names above ~⅔ presence.
